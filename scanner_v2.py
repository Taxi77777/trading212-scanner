...
def us_session_status() -> tuple[bool, str]:
    """Return regular-session status using New York wall-clock time."""
    now_utc = datetime.now(ZoneInfo("UTC"))
    now_ny = now_utc.astimezone(ZoneInfo("America/New_York"))
    open_ = now_ny.weekday() < 5 and ((now_ny.hour > 9 or (now_ny.hour == 9 and now_ny.minute >= 30)) and now_ny.hour < 16)
    status = f"UTC {now_utc:%Y-%m-%d %H:%M} | New York {now_ny:%Y-%m-%d %H:%M %Z}"
    return open_, status
...
def prebreakout(symbol: str, d: base.BarData, spy: base.BarData | None) -> Candidate | None:
    ...
    if price > ema21[-1] and ema21[-1] > ema21[-5]: long_score += 15; long_r.append("EMA21 ascendante")
    if t15 > 0 or t60 > 0: long_score += 10; long_r.append("tendance alignée")
    if 0 <= dist_up <= 0.85: long_score += 20; long_r.append(f"résistance à {dist_up:.2f} ATR")
    if accumulation: long_score += 15; long_r.append("compression + reprise volume")
    elif compression <= 0.82: long_score += 8; long_r.append("compression")
    if higher_lows: long_score += 10; long_r.append("creux ascendants")
    if rs > 0.20: long_score += 10; long_r.append("force relative")
    if 0.5 <= atr20 / price * 100 <= 5: long_score += 5
    if price < ema21[-1] and ema21[-1] < ema21[-5]: short_score += 15; short_r.append("EMA21 descendante")
    if t15 < 0 or t60 < 0: short_score += 10; short_r.append("tendance baissière")
    if 0 <= dist_down <= 0.85: short_score += 20; short_r.append(f"support à {dist_down:.2f} ATR")
    if accumulation: short_score += 12; short_r.append("compression + reprise volume")
    if lower_highs: short_score += 10; short_r.append("sommets descendants")
    if rs < -0.20: short_score += 10; short_r.append("faiblesse relative")
    side = "BUY" if long_score >= short_score else "SELL"
    score = max(long_score, short_score)
    if score < MIN_CANDIDATE: return None
    a = atr20
    if side == "BUY":
        stop = min(price - 1.35 * a, min(d.low[-8:]) - 0.10 * a)
        if not (0 < stop < price): return None
        r = price - stop; reasons = long_r
        tp1, tp2 = price + r, price + 2 * r
    else:
        stop = max(price + 1.35 * a, max(d.high[-8:]) + 0.10 * a)
        if stop <= price: return None
        r = stop - price; reasons = short_r
        tp1, tp2 = price - r, price - 2 * r
    sig = base.Signal(symbol, side, min(100, score), price, stop, tp1, tp2, r, a, a/price*100, vol_ratio, t15lbl, t60lbl, reasons)
...
def macro_overlay(data: dict[str, base.BarData | None], candidates: list[Candidate]) -> tuple[int, str, int, str, int, float, list[str]]:
    ...
    inst=0
    for x,label in [(spy,"SPY"),(qqq,"QQQ")]:
    ...
    sector_vals=[ret(data[e]) for e in macro.SECTOR_ETFS.values() if data.get(e)]
...
def adjust(c: Candidate, ov, data):
    m,regime,inst,ilabel,sec,br,_ = ov
    ...
    c.signal.score=int(max(0,min(100,round(score)))); c.signal.reason=c.signal.reason[-6:]
    if c.mode=="CONFIRMÉ" and c.signal.score<FINAL_MIN: return None
    return c


def text(c,ov):
    s=c.signal; icon="🟢" if s.side=="BUY" else "🔴"; mode="🔥 PRE-BREAKOUT / ACCUMULATION" if c.mode=="PRE-BREAKOUT" else "SIGNAL CONFIRMÉ"
...
def send_once(c,state,now,cooldown,ov):
    key=f"{c.mode}:{c.signal.symbol}:{c.signal.side}"; prev=state.get(key,{}); last=float(prev.get("sent_at",0))
    if now-last<cooldown: return False
    if base.telegram_send(text(c,ov)):
        state[key]={"sent_at":now,"price":c.signal.price,"score":c.signal.score}; return True
    return False


def main() -> int:
    if not base.TELEGRAM_BOT_TOKEN or not base.TELEGRAM_CHAT_ID: return 2
    is_open, clock = us_session_status()
    if not is_open:
    ...
    data=fetch_all(symbols); spy=data.get("SPY"); candidates=[]
    ...
    for c in confirmed[:MAX_ALERTS]:
        if send_once(c,state,now,COOLDOWN,ov): sent+=1
    if not confirmed:
        for c in watches[:WATCH_ALERTS]:
            if send_once(c,state,now,WATCH_COOLDOWN,ov): sent+=1
    base.save_state(state)
    base.telegram_send(f"📊 Scan V2: {sum(x is not None for x in data.values())}/{len(symbols)} données | candidats {len(candidates)} | confirmés {len(confirmed)} | pré-breakout {len(watches)} | macro {ov[1]}\n{clock}")
    log.info("V2 complete: candidates=%d confirmed=%d watches=%d sent=%d | %s",len(candidates),len(confirmed),len(watches),sent,clock)
    return 0
